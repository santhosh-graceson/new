...
# g=st.markdown("<h1 style='font-size:20px;text-align: left; color: #87CEEB;width: 940px;'>VOLUME</h1>", unsafe_allow_html=True)
...
st.markdown("""
    <style> 
    div.css-ocqkz7.e1tzin5v3:nth-child(1),div.css-ocqkz7.e1tzin5v3:nth-child(2),div.css-ocqkz7.e1tzin5v3:nth-child(3),div.css-ocqkz7.e1tzin5v3:nth-child(4){
       column-gap: 0.1rem;
    }
    </style>
        ...
        if n<=160:
            if counter<10:
                ...
                for i in range(10):
                    ...
                    df_Inspiration_Flow = df_Inspiration_Flow.append({"Units":n+i, "Inspiration_Flow": aa[i]}, ignore_index=True)
                    ...
                    df_Volume=df_Volume.append({"Units":n+i, "Volume": bb[i]}, ignore_index=True)
                    chart1.altair_chart((alt.Chart(df_Volume.tail(160)).mark_area(color='#87CEEB').encode(x='Units',y=alt.Y('Volume', scale=alt.Scale(domain=[0,2000])))).properties(width=940,height=250).configure_axis(grid=False).configure_axisY(grid=True))
